...
public class AESUtils {
    ...
    public static void main(String[] args) {
        ...
        int randomKey = new Random().nextInt(9999);
        System.out.println("随机数：" + randomKey);
        String data = "{'fig':1,'message':'该设备不允许登录'}";


        try {
            byte[] encryptData = aesEncryptToBytes(data, String.valueOf(randomKey));
            System.out.println("加密后的数据：" + encryptData);
            String decryptData = aesDecryptByBytes(encryptData, String.valueOf(randomKey));
